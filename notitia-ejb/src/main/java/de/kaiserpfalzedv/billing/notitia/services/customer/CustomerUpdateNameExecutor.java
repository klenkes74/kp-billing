 ...
 */
@ApplicationScoped
public class CustomerUpdateNameExecutor extends BaseExecutor<CustomerUpdateNameCommand> {
    private static final Logger LOG = LoggerFactory.getLogger(CustomerUpdateNameExecutor.class);

    @Override
    public void execute(@Observes final CustomerUpdateNameCommand command) throws CommandFailedException {
        ...
        try {
            if (customer != null) {
                customer.setCostCenter(command.getCustomerName());
                em.merge(customer);

                JPACustomerUpdateNameEvent event = new JPACustomerUpdateNameEvent(command);
                em.persist(event);
            ...
            BUSINESS.error("Can't update customer name (command: {}): customer={}", command.getId(), command.getObjectId());
            OPERATIONS.warn("Can't update customer name (command: {}): customer={}", command.getId(), command.getObjectId());
