 ...
 */
@ApplicationScoped
public class CustomerCreateExecutor extends BaseExecutor<CustomerCreateCommand> {
    private static final Logger LOG = LoggerFactory.getLogger(CustomerCreateExecutor.class);

    @Override
    public void execute(@Observes final CustomerCreateCommand command) throws CommandFailedException {
        JPACustomer jpa = new JPACustomer(command.getData());
        JPACustomerCreateEvent event = new JPACustomerCreateEvent(command);
