 ...
 */
@ApplicationScoped
public class CustomerUpdateBillingEmailExecutor extends BaseExecutor<CustomerUpdateBillingEmailCommand> {
    private static final Logger LOG = LoggerFactory.getLogger(CustomerUpdateBillingEmailExecutor.class);

    @Override
    public void execute(@Observes final CustomerUpdateBillingEmailCommand command) throws CommandFailedException {
        ...
        try {
            ...
            if (customer != null) {
                ...
                JPACustomerUpdateBillingEmailEvent event = new JPACustomerUpdateBillingEmailEvent(command);
                em.persist(event);
