 ...
 */
@ApplicationScoped
public class CustomerUpdateCostCenterExecutor extends BaseExecutor<CustomerUpdateCostCenterCommand> {
    private static final Logger LOG = LoggerFactory.getLogger(CustomerUpdateCostCenterExecutor.class);

    @Override
    public void execute(@Observes final CustomerUpdateCostCenterCommand command) throws CommandFailedException {
        ...
        try {
            if (customer != null) {
                customer.setCostCenter(command.getCostCenter());
                em.merge(customer);

                JPACustomerUpdateCostCenterEvent event = new JPACustomerUpdateCostCenterEvent(command);
                em.persist(event);
            ...
            BUSINESS.error("Can't update cost center (command: {}): customer={}", command.getId(), command.getObjectId());
            OPERATIONS.warn("Can't update cost center (command: {}): customer={}", command.getId(), command.getObjectId());
