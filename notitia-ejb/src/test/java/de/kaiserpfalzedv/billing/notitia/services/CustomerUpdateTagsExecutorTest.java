...
import de.kaiserpfalzedv.billing.notitia.services.customer.CustomerUpdateTagsExecutor;
 ...
 * @since 2018-02-25
 */
public class CustomerUpdateTagsExecutorTest {
    private static final Logger LOG = LoggerFactory.getLogger(CustomerUpdateTagsExecutorTest.class);
    ...
    private static final HashMap<String, String> NEW_TAGS = new HashMap<>();
    ...
    static {
        ...
        NEW_TAGS.put("customer", "54321");
    }


    private CustomerUpdateTagsExecutor service;
    ...
    public void shouldUpdateEmailWhenEventListenerIsCalled() throws CommandFailedException {
        logMethod("simple-call", "Updating tags: {}", NEW_TAGS);

        CustomerUpdateTagsCommand command = new CustomerUpdateTagsCommand(CUSTOMER_ID, NEW_TAGS);
        ...
        verify(em, atLeastOnce()).persist(any(JPACustomerUpdateTagsEvent.class));
    }
    ...
    public void shouldThrowCommandExceptionWhenPersistenceExceptionIsThrownOutsideTransaction() throws CommandFailedException {
        logMethod("persistence-failure-outside-transaction", "Updating tags for failing outside of transaction: {}", NEW_TAGS);

        CustomerUpdateTagsCommand command = new CustomerUpdateTagsCommand(CUSTOMER_ID, NEW_TAGS);


        when(em.find(JPACustomer.class, CUSTOMER_ID)).thenReturn(CUSTOMER);
        doThrow(new PersistenceException()).when(em).persist(any(JPACustomerUpdateTagsEvent.class));
    ...
    public void shouldThrowCommandExceptionWhenPersistenceExceptionIsThrownInsideTransaction() throws CommandFailedException {
        logMethod("persistence-failure-inside-transaction", "Updating tags for failing in transaction: {}", NEW_TAGS);
    ...
    @Before
    public void setUp() {
        service = new CustomerUpdateTagsExecutor();
    ...
    public static void setUpMDC() {
        MDC.put("test", "Update Customer Tags Executor");
