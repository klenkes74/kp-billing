...
import de.kaiserpfalzedv.billing.notitia.services.customer.CustomerUpdateBillingEmailExecutor;
 ...
 * @since 2018-02-25
 */
public class CustomerUpdateBillingEmailExecutorTest {
    private static final Logger LOG = LoggerFactory.getLogger(CustomerUpdateBillingEmailExecutorTest.class);
    ...
    private CustomerUpdateBillingEmailExecutor service;
    ...
    public void shouldUpdateEmailWhenEventListenerIsCalled() throws CommandFailedException {
        ...
        CustomerUpdateBillingEmailCommand command = new CustomerUpdateBillingEmailCommand(CUSTOMER_ID, EMAIL_ADDRESS);
        ...
        verify(em, atLeastOnce()).persist(any(JPACustomerUpdateBillingEmailEvent.class));
    }
    ...
    public void shouldThrowCommandExceptionWhenPersistenceExceptionIsThrownOutsideTransaction() throws CommandFailedException {
        ...
        when(em.find(JPACustomer.class, CUSTOMER_ID)).thenReturn(CUSTOMER);
        doThrow(new PersistenceException()).when(em).persist(any(JPACustomerUpdateBillingEmailEvent.class));
    ...
    @Before
    public void setUp() {
        service = new CustomerUpdateBillingEmailExecutor();
